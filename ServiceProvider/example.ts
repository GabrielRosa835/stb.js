...
const createContainer = defineServiceContainer<AppServices>({
    
    singletonDb: addSingleton((sp) => {
        singletonCounter++;
    ...
    appMain: addTransient((sp) => {
        // Demonstrating classic resolution inside a factory
        const db = sp.resolve<DatabaseService>('singletonDb');
        const ctx = sp.resolve<RequestContext>('scopedContext');
    ...
    circleA: addTransient((sp) => sp.resolve('circleB')),
    circleB: addTransient((sp) => sp.resolve('circleA'))
});
...
async function runTests() {
    ...
    const { singletonDb: db1_again, scopedContext: ctx1_again, transientRng: rng1_again } = scope1;
    ...
    const scope2 = createContainer();

    logAction("Testing Cross-Scope Behavior...");
    const { singletonDb: db2, scopedContext: ctx2 } = scope2;
